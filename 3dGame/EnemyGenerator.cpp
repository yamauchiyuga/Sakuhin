...
// コンストラクタ
EnemyGenerator::EnemyGenerator(IWorld* world, const std::string& csv_stage_data, const std::string& csv_fase_pos) {
	world_ = world;
	...
	if (fase_pos_[next_fase_] > player_->transform().position().z) {	
		while (statge_data_.front().first == next_fase_) {
			...
			if (EnemyName == "Skeleton") {
				world_->add_actor(new Skeketon{ world_,RowData.second.EnemyPos });
			}
			if (EnemyName == "Witch") {
				world_->add_actor(new Witch{ world_,RowData.second.EnemyPos });
			}
			if (EnemyName == "Dragon") {
				world_->add_actor(new Dragon{ world_,RowData.second.EnemyPos });
			}
