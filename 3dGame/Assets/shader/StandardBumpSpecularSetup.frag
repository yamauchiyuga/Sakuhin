...
void main() {
    // ディフューズマップカラー
    vec4 diffuseColor = texture(gs_DiffseTexture, v_TexCoord) * gs_MaterialDiffuse;
    diffuseColor.rgb = ConvertToLinear(diffuseColor.rgb);
    // スペキュラカラー
    vec4 specularColor = texture(gs_SpecularTexture, v_TexCoord) * gs_MaterialSpecular;
    specularColor.rgb = ConvertToLinear(specularColor.rgb);
    // スムースネスをラフネスに変換 0.0(ツルツル）～1.0(ザラザラ）
    float perceptualRoughness = SmoothnessToPerceptualRoughness(specularColor.a);
    ...
    // 双方向反射率分布関数(BRDF)の計算
    vec3 Fd = DiffuseBRDF(diffuseColor.rgb);
    vec3 Fr = SpecularBRDF(specularColor.rgb, NoH, NoV, NoL, LoH, roughness);
    vec3 BRDF = (Fd + Fr) * irradiance;
    // イメージベースドライティング
    vec3 IBL = ImageBasedLighting(N, R, NoV, diffuseColor.rgb, specularColor.rgb, perceptualRoughness);
    ...
    // 最終結果を計算
    out_FragColor = vec4(finalColor, diffuseColor.a * gs_Color.a);
}
